...
import com.focus.common.annotation.NoSignCheck;
import com.focus.common.constant.FocusRedisKey;
...
public class SignInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (handler instanceof HandlerMethod handlerMethod) {
            NoSignCheck signAnnotation = handlerMethod.getMethodAnnotation(NoSignCheck.class);
        ...
        if (redisUtil.get(FocusRedisKey.SIGN_REPLAY.value() + nonce) != null) {
            throw new FocusException(FocusResultCode.PERMISSION_EXCEPTION);
        }

        redisUtil.set(FocusRedisKey.SIGN_REPLAY.value() + nonce, 1, SIGN_REPLAY_EXPIRE_TIME);
